...
2. `internal/app.InitializeContainer()` boots ent/Postgres, Redis cache, JWT service, Redis idempotency store, repositories, and handler stacks (generated by Google Wire).
...
- New dependencies: add a port under `internal/interface/core/ports`, implement in `internal/infrastructure`, wire in `internal/app.InitializeContainer()` (regenerate Wire).
